defmodule BlogsCrawler.FetcherTest do
  use MediaRadar.DataCase
  use ExUnit.Case, async: true

  alias MediaRadar.BlogsCrawler.Fetcher
  alias MediaRadar.Song

  import MediaRadar.Factory

  setup context do
    bypass = Bypass.open(port: 1234)
    feed_url = "http://localhost:#{bypass.port}"

    response = File.read! "test/media_radar/fixtures/songs_fetching/#{context[:fixture]}"
  ...
  test "fetch_songs/1 from crawled rss links", %{bypass: bypass, blog: blog} do
    Enum.each ~w(/one /two /three), fn (entry_path) ->
      entry_page = File.read! "test/media_radar/fixtures/songs_fetching/blog_pages/#{entry_path}.html"
