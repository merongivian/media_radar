defmodule BlogsCrawler.FetcherTest do
  use JamRadar.DataCase
  use ExUnit.Case, async: true

  alias JamRadar.BlogsCrawler.Fetcher
  alias JamRadar.Song
  ...
  setup context do
    ...
    response = File.read! "test/jam_radar/fixtures/songs_fetching/#{context[:fixture]}"
  ...
  test "fetch_songs/1 from crawled rss links", %{bypass: bypass, blog: blog} do
    Enum.each ~w(/one /two /three), fn (entry_path) ->
      entry_page = File.read! "test/jam_radar/fixtures/songs_fetching/blog_pages/#{entry_path}.html"
